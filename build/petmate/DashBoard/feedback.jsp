...
<%
    ...
    if (request.getParameter("deleteId") != null) {
        ...
        int status = UserDAO.deleteFeedback(deleteId);
        if (status > 0) {
            out.print("<script>alert('Feedback deleted successfully'); window.location='feedback.jsp';</script>");
        } else {
            out.print("<script>alert('Failed to delete feedback');</script>");
        }
    }

    if ("POST".equalsIgnoreCase(request.getMethod())) {
        String message = request.getParameter("message");
        int status = UserDAO.insertFeedback(uname, message);
        if (status > 0) {
            out.print("<script>alert('Feedback submitted successfully'); window.location='feedback.jsp';</script>");
        } else {
            out.print("<script>alert('Failed to submit feedback');</script>");
        }
    }
%>
...
<nav class="navbar navbar-expand-lg navbar-dark" style="background-color: rgba(44, 90, 160, 0.95); backdrop-filter: blur(10px);">
	  <div class="container-fluid">
	    <a class="navbar-brand fw-bold fs-3" href="#">
	        <i class="fas fa-paw me-2"></i>PetMate - Feedback
	    </a>
	    ...
	    <div class="collapse navbar-collapse" id="navbarNav">
	      <ul class="navbar-nav ms-auto">
	        ...
	        <li class="nav-item">
	          <a class="nav-link" href="adoptpet.jsp">Adopt</a>
	        </li>
	        <li class="nav-item"><a class="nav-link active" href="feedback.jsp">Feedbacks</a></li>
	        <li class="nav-item">
	          <a class="nav-link">
	          <%= 
...
<div class="container mt-5">
    ...
    <form method="post" action="feedback.jsp">
        <div class="mb-3">
            <textarea name="message" class="form-control" rows="3" placeholder="Write your feedback here..." required></textarea>
        </div>
    ...
    <h3 class="mb-4">All Feedback</h3>
    <div class="list-group">
        <%
            ResultSet rs = UserDAO.getAllFeedback();
            while (rs.next()) {
        %>
        <div class="list-group-item">
            <div class="d-flex w-100 justify-content-between">
                <h6 class="mb-1"><%= rs.getString("username") %></h6>
                <small><%= rs.getString("submitted_at") %></small>
            </div>
            <p class="mb-1"><%= rs.getString("message") %></p>

            <% if (uname.equals(rs.getString("username"))) { %>
                <form method="get" action="feedback.jsp" onsubmit="return confirm('Delete this feedback?');">
